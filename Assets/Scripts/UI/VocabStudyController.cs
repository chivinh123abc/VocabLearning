...
namespace VocabLearning.UI
{
    ...
    public partial class VocabStudyController : MonoBehaviour
    {
        ...
        private void AutoPartitionSetLevels(VocabLearning.Data.VocabSetJson set)
        {
            ...
            if (autoGeneratedLevels.Count > 1)
            {
                set.difficulty = "MultiLevel";
            }
            else if (autoGeneratedLevels.Count == 1)
            {
                set.difficulty = autoGeneratedLevels[0].difficulty;
            }
        }
            ...
            if (_jsonDb != null && _jsonDb.vocabSets != null && _jsonDb.vocabSets.Count > 0)
            {
                ...
                if (listContainer != null)
                {
                    ...
                    foreach (var set in _jsonDb.vocabSets)
                    {
                        ...
                        // [MODIFIED] Logic hiển thị Label Cấp độ và Số từ phù hợp với hệ thống mới (Chỉ hiển thị "Đa cấp độ" khi có từ 2 Cấp độ trở lên)
                        string displayDiff = set.difficulty;
                        string displayCountText = $"{set.wordCount} từ";

                        if (set.levels != null && set.levels.Count > 1)
                        {
                            displayDiff = "Đa cấp độ";
                            displayCountText = $"{set.levels.Count} Cấp độ";
                            ...
                            if (_jsonDb.currentUser != null && _jsonDb.currentUser.savedSetLevels != null)
                            {
                                ...
                                if (saved != null)
                                {
                                    string svLevel = saved.level;
                                    if (svLevel.Equals("Easy", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Dễ";
                                    else if (svLevel.Equals("Medium", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Trung bình";
                                    else if (svLevel.Equals("Hard", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Khó";
                                    else displayDiff = svLevel;

                                    // Cập nhật số từ của level đó luôn
                                    var levelData = set.levels.Find(l => l.difficulty == saved.level);
                                    if (levelData != null) displayCountText = $"{levelData.wordIds.Count} từ";
                                }
                            }
                        }
                        else
                        {
                            // Nếu chỉ có 1 cấp độ hoặc không chia cấp độ, hiển thị theo độ khó của set
                            string rawDiff = set.difficulty;
                            if (set.levels != null && set.levels.Count == 1)
                            {
                                rawDiff = set.levels[0].difficulty;
                            }

                            if ((rawDiff ?? "").Equals("Easy", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Dễ";
                            else if ((rawDiff ?? "").Equals("Medium", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Trung bình";
                            else if ((rawDiff ?? "").Equals("Hard", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Khó";
                            else if ((rawDiff ?? "").Equals("MultiLevel", System.StringComparison.OrdinalIgnoreCase) || (rawDiff ?? "").Equals("Multi-Level", System.StringComparison.OrdinalIgnoreCase)) displayDiff = "Đa cấp độ";
                            else displayDiff = rawDiff;

                            int wordCountVal = set.wordCount;
                            if (set.levels != null && set.levels.Count == 1) wordCountVal = set.levels[0].wordIds.Count;
                            else if (set.wordIds != null) wordCountVal = set.wordIds.Count;
                            displayCountText = $"{wordCountVal} từ";
                        }
        ...
        private void CreateLevelButtons()
        {
            ...
            else
            {
                ...
                // Tính tiến độ cho level mặc định này 
                int masteredCount = 0;
